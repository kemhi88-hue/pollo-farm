...
start_tunnel() {
    ...
    echo -e "${CYAN}    localhost:${LOCAL_PORT} → ${REMOTE_ADB}${NC}"
...
install_apk() {
    echo -e "${CYAN}[*] Kiểm tra APK...${NC}"

    # Kiểm tra đã cài chưa
    if adb -s $SERIAL shell pm list packages 2>/dev/null | grep -q "$PACKAGE"; then
        echo -e "${GREEN}[OK] $PACKAGE đã cài sẵn${NC}"
        return 0
    fi

    echo -e "${YELLOW}[!] App chưa cài, bắt đầu tải + cài...${NC}"

    # Tải APK nếu chưa có
    if [ ! -f "$APK_FILE" ]; then
        echo -e "${CYAN}[*] Đang tải APK...${NC}"
        echo -e "${CYAN}    URL: $APK_URL${NC}"

        local retry=0
        while [ $retry -lt 3 ]; do
            curl -L -o "$APK_FILE" "$APK_URL" --progress-bar 2>&1

            if [ -f "$APK_FILE" ]; then
                local FSIZE=$(wc -c < "$APK_FILE" 2>/dev/null)
                if [ "$FSIZE" -gt 1000000 ]; then
                    echo -e "${GREEN}[OK] Tải xong: $(( FSIZE / 1048576 )) MB${NC}"
                    break
                else
                    echo -e "${YELLOW}[!] File quá nhỏ (${FSIZE} bytes), thử lại...${NC}"
                    rm -f "$APK_FILE"
                fi
            fi

            retry=$((retry+1))
            echo -e "${YELLOW}[!] Retry tải APK $retry/3...${NC}"
            sleep 3
        done

        if [ ! -f "$APK_FILE" ]; then
            echo -e "${RED}[FAIL] Không tải được APK sau 3 lần${NC}"
            return 1
        fi
    else
        local FSIZE=$(wc -c < "$APK_FILE" 2>/dev/null)
        echo -e "${GREEN}[OK] APK đã có sẵn: $(( FSIZE / 1048576 )) MB${NC}"
    fi

    # Cài APK qua ADB
    echo -e "${CYAN}[*] Đang cài APK lên thiết bị...${NC}"
    echo -e "${CYAN}    (có thể mất 1-3 phút tùy kết nối)${NC}"

    local INSTALL_OUT
    INSTALL_OUT=$(adb -s $SERIAL install -r -g "$APK_FILE" 2>&1)
    local INSTALL_RET=$?

    if [ $INSTALL_RET -eq 0 ] && echo "$INSTALL_OUT" | grep -qi "success"; then
        echo -e "${GREEN}[OK] Cài APK thành công!${NC}"

        # Verify
        sleep 2
        if adb -s $SERIAL shell pm list packages 2>/dev/null | grep -q "$PACKAGE"; then
            echo -e "${GREEN}[OK] Verified: $PACKAGE đã cài${NC}"
            return 0
        else
            echo -e "${YELLOW}[!] Cài xong nhưng không tìm thấy package${NC}"
            return 1
        fi
    else
        echo -e "${RED}[FAIL] Cài APK thất bại${NC}"
        echo -e "${RED}       $INSTALL_OUT${NC}"

        # Thử cách 2: push + pm install
        echo -e "${YELLOW}[!] Thử cách 2: push + pm install...${NC}"
        adb -s $SERIAL push "$APK_FILE" /data/local/tmp/pollo.apk 2>/dev/null
        sleep 1
        local PM_OUT
        PM_OUT=$(adb -s $SERIAL shell pm install -r -g /data/local/tmp/pollo.apk 2>&1)

        if echo "$PM_OUT" | grep -qi "success"; then
            echo -e "${GREEN}[OK] Cài APK thành công (cách 2)!${NC}"
            adb -s $SERIAL shell rm /data/local/tmp/pollo.apk 2>/dev/null
            return 0
        else
            echo -e "${RED}[FAIL] Cài APK thất bại cả 2 cách${NC}"
            echo -e "${RED}       $PM_OUT${NC}"
            return 1
        fi
    fi
}
...
get_otp() {
    ...
    while [ $attempt -lt 50 ]; do
        ...
        echo -e "  ${YELLOW}No OTP yet: ${F_TXT:0:100}${NC}" >&2
        sleep 3
    done
...
echo -e "${GREEN}║   SSH + APK + OTP AUTO            ║${NC}"
echo -e "${GREEN}║                                   ║${NC}"
...
curl -s --max-time 10 https://api.mail.tm/domains >/dev/null 2>&1 || {
    echo -e "${RED}[FAIL] mail.tm unreachable${NC}"; exit 1
}
